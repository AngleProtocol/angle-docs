...
AgEUR and ANGLE can be bridged between networks using Angle's infrastructure, compatible with multiple bridge solutions. Bridging these tokens is available natively on the [Angle App](https://app.angle.money/#/bridges-agEUR).
...
To limit the risk associated with each bridge, a total and hourly cap on the quantity of each token that can be bridged to & from specific networks has been implemented. This guarantees that if a bridge was to be hacked, then the exposure of the protocol would remain limited to what can be taken in a couple of hours.
